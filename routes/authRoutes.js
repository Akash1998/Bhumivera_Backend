...
router.get("/profile",authenticateAdmin,async(req,res)=>{res.json({message:"Profile access"});});
...
router.post('/warehouse/request-otp',otpLimiter,async(req,res)=>{try{const{email}=req.body;if(!email)return res.status(400).json({message:'Email required'});const[a]=await pool.query('SELECT id,email,role FROM admin_users WHERE email=? AND role IN (?,?)',[email,'warehouse_admin','superadmin']);let t=a[0],iu=!1;if(!t){const[u]=await pool.query('SELECT u.id,u.email,u.role,wa.is_active FROM users u JOIN warehouse_access wa ON u.id=wa.user_id WHERE u.email=? AND wa.is_active=1',[email]);if(u.length>0){t=u[0];iu=!0}}if(!t)return res.status(404).json({message:'No warehouse permission found for this account.'});const o=Math.floor(100000+Math.random()*900000).toString(),e=new Date(Date.now()+600000);if(iu){await pool.query('UPDATE users SET reset_otp=?, reset_otp_expires=? WHERE email=?',[o,e,email])}else{await pool.query('UPDATE admin_users SET login_otp=?, login_otp_expires=? WHERE email=?',[o,e,email])}await sendMail({to:email,subject:'Warehouse Login OTP',html:`<p>Your warehouse login OTP is: <strong>${o}</strong></p><p>Expires in 10 minutes. Do not share this code.</p>`});res.json({message:'OTP sent to your email.'})}catch(err){res.status(500).json({message:err.message})}});
router.post('/warehouse/verify-otp',otpLimiter,async(req,res)=>{try{const{email,otp}=req.body;if(!email||!otp)return res.status(400).json({message:'Email and OTP required'});const[a]=await pool.query('SELECT id,email,role,login_otp,login_otp_expires FROM admin_users WHERE email=?',[email]);let w=a[0],iu=!1;if(!w||!['warehouse_admin','superadmin'].includes(w.role)){const[u]=await pool.query('SELECT u.id,u.email,u.role,u.reset_otp as login_otp,u.reset_otp_expires as login_otp_expires FROM users u JOIN warehouse_access wa ON u.id=wa.user_id WHERE u.email=? AND wa.is_active=1',[email]);if(u.length>0){w=u[0];iu=!0}}if(!w)return res.status(404).json({message:'Account not authorized.'});if(!w.login_otp||w.login_otp!==otp)return res.status(401).json({message:'Invalid OTP.'});if(new Date()>new Date(w.login_otp_expires))return res.status(401).json({message:'OTP expired.'});if(iu){await pool.query('UPDATE users SET reset_otp=NULL, reset_otp_expires=NULL WHERE email=?',[email])}else{await pool.query('UPDATE admin_users SET login_otp=NULL, login_otp_expires=NULL WHERE email=?',[email])}const t=jwt.sign({id:w.id,email:w.email,role:w.role||'distributor'},process.env.JWT_SECRET,{expiresIn:'7d'});res.json({token:t,admin:{id:w.id,email:w.email,role:w.role||'distributor'}})}catch(err){res.status(500).json({message:err.message})}});
